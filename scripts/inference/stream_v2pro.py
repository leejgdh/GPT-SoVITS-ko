...
def _init_text_preprocessor(device: str = "cuda", bert_path: str = "GPT_SoVITS/pretrained_models/chinese-roberta-wwm-ext-large"):
    ...
    if _text_preprocessor is None:
        ...
        _text_preprocessor = TextPreprocessor(bert_model, tokenizer, torch.device(device))
    return _text_preprocessor

def get_phones_and_bert(text: str, language: str, version: str):
    tp = _init_text_preprocessor()
    return tp.get_phones_and_bert(text, language, version)
...
class StreamT2SModel(nn.Module):
    ...
    def pre_infer(
        ...
        x_attn_mask_pad = F.pad(
            x_attn_mask,
            (0, y_len),  # xx 영역의 0을 (x, x+y) 크기로 확장하여 xy 영역은 1로 패딩
            value=True,
        )
        y_attn_mask = F.pad(  # yy 우상단 1 마스크를 왼쪽 xy 영역 0으로 확장, (y, x+y)
    ...
    def decode_next_token(
        ...
        if idx < 11:  # 최소 10개 토큰은 생성해야 정지 허용 (약 0.4초)
            logits = logits[:, :-1]
...
def find_best_audio_offset_fast(reference_audio: Tensor, search_audio: Tensor):
    ...
    if search_len < ref_len:
        raise ValueError(
            f"검색 오디오 길이({search_len})는 참조 오디오 길이({ref_len}) 이상이어야 합니다"
        )

    # F.conv1d로 교차상관(cross-correlation) 계산
...
def test_stream(
    ...
    text_seq_id, text_bert_T, norm_text = get_phones_and_bert(
        "This is a simple example. It is really amazing that it was completed so easily. Next, let us talk about foxes. They have long tails and pointed ears. In legends, they even have nine tails. Do you find foxes amazing?", "auto", "v2"
    )
...
def export_prov2(
    ...
    text_seq_id, text_bert_T, norm_text = get_phones_and_bert(
        "This is a simple example. The King and His Stories. Once there was a king. He likes to write stories, but his stories were not good.", "auto", "v2"
    )
    text_seq = torch.LongTensor([text_seq_id]).to(device)
    text_bert = text_bert_T.T
    if is_half:
